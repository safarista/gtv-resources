...
gtv.jq.VideoControl.prototype.makeControl = function(videoControlParms) {
	...
	if (videoControl.behaviors.allowWindowStateChange || videoControl.behaviors.startFullScreen) {
		$(window).resize(function() {
			if (videoControl.isFullScreen) {
				var zoom = gtv.jq.GtvCore.getZoom();
				videoControl.container.css('height', window.innerHeight/zoom + 'px').css('width', window.innerWidth/zoom + 'px');
			}
		});
	}
	...
	if (!videoControl.behaviors.showControls) {
	    ...
	    videoControl.keyController.addBehaviorZone(videoControl.behaviorZone, false);
	   	...
	   	if (videoControl.selectors.fullScreenVideoCommandsParent) {
		    ...
		    videoControl.fullScreenBehaviorZone = new gtv.jq.KeyBehaviorZone(zoneParms);
		    videoControl.keyController.addBehaviorZone(videoControl.fullScreenBehaviorZone, false, [gtv.jq.VideoControl.fullScreenLayer]);
		}
	}
 ...
 * @param {Object} object containing the informacion from the video to be played.
 */
gtv.jq.VideoControl.prototype.showVideo = function(videoInfo) {
	...
	videoControl.container = $('<div></div>').attr('id', videoControl.containerId);
	...
	$(video).addClass(videoControl.styles.video).attr('width', '100%').attr('height', '100%');
	...
	$(video).bind('timeupdate', function() {
		...
		if (videoControl.control.buffered && videoControl.control.buffered.length > 0) {
	...
	$(video).bind('ended', function() {
		if (videoControl.behaviors.exitFullScreenOnEnded && videoControl.isFullScreen) {
