...
gtv.jq.TemplatePage.prototype.makeTopMenu = function() {
	...
	templatePage.keyController.addBehaviorZone(templatePage.topMenuZone, true, ['default', gtv.jq.VideoControl.fullScreenLayer]);
}
...
gtv.jq.TemplatePage.prototype.makeVideoControl = function() {
	...
	var callbacks = {
		...
		timeUpdated: function() {
			...
			templatePage.fullScreenProgressBar.setElapsedTime(templatePage.videoControl.getElapsedTime());
		},
		loaded: function() {
		...
		onLoadProgress: function() {
			...
			templatePage.fullScreenProgressBar.setLoadedTime(templatePage.videoControl.getLoadedTime());
		}
	};
...
gtv.jq.TemplatePage.prototype.makeSlider = function() {
	...
	if (!templatePage.item || !templatePage.item.videos || templatePage.item.videos.length == 0)
		return;
 ...
 */
gtv.jq.TemplatePage.prototype.fadeControlsIn = function(avoidStart, onlyControls) {
	var templatePage = this;
	...
	$('#carouselFS').fadeIn(200, avoidStart?function() {}:function() {
		templatePage.startFadeTimeout();
	});
}
...
gtv.jq.TemplatePage.prototype.start = function() {
	...
	templatePage.keyController.start(null, true, gtv.jq.VideoControl.fullScreenLayer);
}
