...
gtv.jq.Tooltip.prototype.buildCanvas = function(element) {
	...
	var currentX = pos.left + element.innerWidth()/2 - currentWidth/2 - this.behaviors.glowPadding;
	var diff = 0;
	if (currentX < 0) {
		diff = -currentX + 3;
		currentX = 3;
		...
		var tempDiff = $(window).width()/zoom - currentX - currentWidth - this.behaviors.glowPadding * 2;
		if (tempDiff < 0) {
			diff = tempDiff - 3;
	...
	// build canvas bg
	this.canvasBg.attr('width', (currentWidth + this.behaviors.glowPadding*2)).attr('height', (currentHeight + this.behaviors.glowPadding*2));
	// build canvas context
	var ctx = document.getElementById('tooltipBg').getContext('2d');
	// give some padding
	ctx.translate(this.behaviors.glowPadding, this.behaviors.glowPadding); // translate shape in order to give margin to the glow to be shown
	// begin fill
	ctx.beginPath();
	ctx.lineWidth = 2;
	...
	ctx.quadraticCurveTo((currentWidth + this.behaviors.radius), 0, (currentWidth + this.behaviors.radius), this.behaviors.radius);
	// draw right side
	ctx.lineTo((currentWidth + this.behaviors.radius), currentHeight);
	// draw bottom-right corner
	ctx.quadraticCurveTo((currentWidth + this.behaviors.radius), (currentHeight + this.behaviors.radius), currentWidth, (currentHeight + this.behaviors.radius));
	if (this.behaviors.direction == 'up') {
		// draw bottom tip from right to left
		ctx.lineTo(((currentWidth + this.behaviors.radius)/2 + 10 - diff), (currentHeight + this.behaviors.radius));
		ctx.lineTo((currentWidth + this.behaviors.radius)/2 - diff, (currentHeight + this.behaviors.radius) + 10);
		ctx.lineTo(((currentWidth + this.behaviors.radius)/2 - 10 - diff), (currentHeight + this.behaviors.radius));
	...
	ctx.quadraticCurveTo(0, (currentHeight + this.behaviors.radius), 0, currentHeight);
	// draw left side
	...
	var finalY = null;
	if (this.behaviors.direction == 'up') {
		currentY = element.offset().top - currentHeight - this.behaviors.glowPadding*2 + 10;
		finalY = currentY - 15;
