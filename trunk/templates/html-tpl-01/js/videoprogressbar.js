...
gtv.jq.VideoProgressBar.prototype.makeControl = function(videoProgressBarParms)
{
	...
	progressBar.container = $('<div></div>').attr('id', progressBar.containerId)
									.addClass('progressbar-control ' + progressBar.styles.container);
	progressBar.elapsedTime = $('<div></div>').addClass('progress-elapsed-time ' + progressBar.styles.elapsedTime)
											  .html('00:00');
	progressBar.progressBack = $('<div></div>').addClass('progress-bar-back ' + progressBar.styles.progressBack);
	progressBar.playPogress = $('<div></div>').css('width', '0px')
											  .addClass('progress-play-progress ' + progressBar.styles.playProgress);
	progressBar.loadPogress = $('<div></div>').css('width', '0px')
											  .addClass('progress-load-progress ' + progressBar.styles.loadProgress);
	progressBar.duration = $('<div></div>').addClass('progress-duration ' + progressBar.styles.duration)
											  .html('00:00');
	...
	progressBar.progressBack.bind('mousemove', function(e) {
		if (progressBar.intDuration > 0) {
			...
			progressBar.timeTooltip.html(progressBar.formatSeconds(progressBar.intDuration * (e.clientX/zoom - backOffset.left) / backWidth));
	...
	progressBar.progressBack.bind('click', function(e) {
		if (typeof progressBar.callbacks.onTimeSelected == 'function') {
			...
			var seconds = progressBar.intDuration * (e.clientX/zoom - backOffset.left) / backWidth;
			
			progressBar.callbacks.onTimeSelected(seconds);
		}
	});
	
	progressBar.progressBack.append(progressBar.playPogress).append(progressBar.loadPogress);
...
gtv.jq.VideoProgressBar.prototype.formatSeconds = function(seconds) {
	return gtv.jq.GtvCore.formatTime(seconds, (seconds >= 3600)?'hours':'minutes');
};
